...
class LoginViewModel : ViewModel() {
    ...
    private fun reduce(state : LoginState, intent: LoginIntent) : LoginState {
        return when(intent)
        {
            is LoginIntent.ChangeUsername ->{state.copy(username = intent.username)}
            is LoginIntent.ChangePassword ->{state.copy(password = intent.password)}
            else ->{state}
        }
    }
    private fun login()
    ...
    private fun changePassword(password: String)
    {
        ...
        _uiState.update { currentState -> currentState.copy(password = password,
            isValidPassword = if(verification is ValidationResult.Success) true else false,
     ...
     private fun changeUsername(username: String)
    {
        viewModelScope.launch { //za slucaj da bude trebala provera u databazi, al mi nelogicno malo
            ...
            _uiState.update { currentState -> currentState.copy(username = username,
                isValidUsername = if(verification is ValidationResult.Success) true else false,
